...
public class Drawable {
    ...
    private void processGetImageRequest(Client client, byte format) throws IOException {
        ...
        if (format == Z_PIXMAP_FORMAT) {
            n = (_depth >= 24) ? wh * 4 : wh * ((_depth + 7) / 8);
        ...
        synchronized (io) {
            ...
            if (format == 2) {
                for (int i = 0; i < wh; i++) {
                    int px = pixels[i] & planeMask;
                    if (_depth >= 24) {
                        io.writeInt(px); // CARD32 — byte order handled by writeInt
                    } else {
                        io.writeByte((byte) (px & 0xff));
                        io.writeByte((byte) ((px >> 8) & 0xff));
                        io.writeByte((byte) ((px >> 16) & 0xff));
                    }
                }
            } else {
    ...
    private boolean processPutImage(Client client, GContext gc, byte format, int bytesRemaining) throws IOException {
        ...
        Log.e("PutImage", "fmt=" + format + " w=" + width + " h=" + height + " depth=" + depth + " _depth=" + _depth + " bytes=" + bytesRemaining);
        ...
        if (format == Z_PIXMAP_FORMAT) {
            rightPad = 0;
            if (depth >= 24) {
                // Try 4 bytes per pixel first, then 3
                n = 4 * width * height;
                ...
                // 1 byte per pixel
                n = width * height;
                // rows padded to scanline pad
                int rowBytes = (width + 3) & ~3; // pad each row to 4 bytes
                int paddedN = rowBytes * height;
                if (bytesRemaining == paddedN) {
                    n = paddedN;
                }
            } else if (depth == 16) {
                n = 2 * width * height;
                ...
                n = ((width + 7) / 8) * height;
                int pad1 = ((-n) & 3);
                if (bytesRemaining != n + pad1) {
                    isShapeMask = true;
                    n = (width + 1) / 2 * height;
                }
            } else {
                n = (width * height * depth + 7) / 8;
            }
        ...
        if (format == BITMAP_FORMAT) {
        ...
        } else if (depth >= 24) {    // 24/32-bit ZPixmap.
            boolean fourBytes = (n == 4 * width * height);
            for (int i = 0; i < colors.length; i++) {
                int r, g, b, a;
                if (fourBytes) {
                    // 32bpp: read as CARD32 (byte-order aware) then extract via visual masks
                    int pixel = io.readInt();
                    r = (pixel >> 16) & 0xff;
                    g = (pixel >> 8) & 0xff;
                    b = pixel & 0xff;
                    a = (pixel >> 24) & 0xff;
                } else {
                    // 24bpp: 3 raw bytes in LSB order (B, G, R)
                    b = io.readByte();
                    g = io.readByte();
                    r = io.readByte();
                    a = 0xff;
                ...
                // In premultiplied ARGB, alpha=0 requires R=G=B=0 (fully transparent).
                // If RGB is non-zero with alpha=0, this is XRGB data (e.g. openbox title bars)
                // where the alpha byte is just padding. Fix alpha to fully opaque.
                if (a == 0 && (r | g | b) != 0) a = 0xff;

                // Cairo/GTK sends premultiplied data (R,G,B already scaled by alpha).
                // Android setPixels() premultiplies again. Un-premultiply to prevent
                // double-darkening of semi-transparent content (GTK sidebar text, toolbar).
                if (fourBytes && a > 0 && a < 255) {
        ...
        } else if (depth == 8) {    // 8-bit ZPixmap (grayscale/indexed).
            int rowBytes = (n == ((width + 3) & ~3) * height) ? ((width + 3) & ~3) : width;
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    int v = io.readByte() & 0xff;
                    colors[y * width + x] = 0xff000000 | (v << 16) | (v << 8) | v;
                }
                // Skip row padding
            ...
            io.readSkip(pad);
            int dx8 = (int) dstX, dy8 = (int) dstY;
            int cw8 = Math.min(width, _bitmap.getWidth() - dx8);
            int ch8 = Math.min(height, _bitmap.getHeight() - dy8);
            if (dx8 >= 0 && dy8 >= 0 && cw8 > 0 && ch8 > 0) {
                _bitmap.setPixels(colors, 0, width, dx8, dy8, cw8, ch8);
            }
            return true;
        ...
        // Write pixels directly to bitmap — bypass Canvas premultiplication.
        // X11 PutImage writes raw pixel data; Android premultiplication would
        // destroy RGB values when alpha=0 (openbox XRGB title bars).
        int dx = (int) dstX;
        int dy = (int) dstY;
        int bw = _bitmap.getWidth();
        int bh = _bitmap.getHeight();
        ...
        if (dx + cw > bw) cw = bw - dx;
        if (dy + ch > bh) ch = bh - dy;

        if (cw > 0 && ch > 0) {
            _bitmap.setPixels(colors, sy * width + sx, width, dx, dy, cw, ch);
        }
