                ...
                default:
                    if (opcode < 0) {
                        ...
                        if (extension != null)
                            ...
                            Log.w("Winlator", String.format("handleNormalRequest: unhandled opcode=%d, requestData=%d, requestLength=%d", opcode, requestData, requestLength));

                    }
                    else Log.w("Winlator", "handleNormalRequest: unsupported opcode " + opcode);
                    break;
            }
        }
        catch (XRequestError e) {
            client.skipRequest();
            Log.e("Winlator", String.format("handleNormalRequest: XRequestError for opcode=%d: %s", opcode, e));
