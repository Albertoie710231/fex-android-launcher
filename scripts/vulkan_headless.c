...
// Always log errors and important state changes — also write to file for debugging
// (logcat truncates large reads, so this ensures we see the full output)
static FILE* g_logfile = NULL;
static void xcb_log_init(void) {
    if (!g_logfile) {
        g_logfile = fopen("/tmp/xcb_bridge.log", "a");
        if (g_logfile) setbuf(g_logfile, NULL); // unbuffered
    }
}
#define ERROR_LOG(...) do { \
    fprintf(stderr, __VA_ARGS__); fflush(stderr); \
    xcb_log_init(); \
    if (g_logfile) { fprintf(g_logfile, __VA_ARGS__); } \
} while(0)
...
// Win32 surface create info (Wine passes this through winevulkan)
...
// Handle to the REAL libvulkan.so.1 (FEX thunked). Saved when Wine dlopen's it.
// RTLD_NEXT can't find symbols from dlopen'd-without-RTLD_GLOBAL libraries,
// so we need the explicit handle for dlsym() to resolve real Vulkan functions.
...
// Create Win32 surface - create a headless-like surface (Wine/DXVK path)
static VkResult my_vkCreateWin32SurfaceKHR(
    VkInstance instance,
    ...
    VkSurfaceKHR* pSurface)
{
    ...
    fprintf(stderr, "[XCB-Bridge] vkCreateWin32SurfaceKHR called (hwnd=%p)\n",
            pCreateInfo ? pCreateInfo->hwnd : NULL);

    VkSurfaceKHR handle = g_next_handle++;
    SurfaceEntry* entry = add_surface(handle, 0, NULL, 0, 0, 1920, 1080);
    if (!entry) return VK_ERROR_OUT_OF_HOST_MEMORY;

    g_current_instance = instance;
    *pSurface = handle;
    fprintf(stderr, "[XCB-Bridge] Created Win32 surface: 0x%lx (headless path)\n",
            (unsigned long)handle);
    return VK_SUCCESS;
}

// Win32 presentation support - always return true
static VkBool32 my_vkGetPhysicalDeviceWin32PresentationSupportKHR(
    VkPhysicalDevice physicalDevice,
    uint32_t queueFamilyIndex)
{
    (void)physicalDevice; (void)queueFamilyIndex;
    fprintf(stderr, "[XCB-Bridge] vkGetPhysicalDeviceWin32PresentationSupportKHR -> VK_TRUE\n");
    return VK_TRUE;
}
    ...
    VkExtensionProperties* pProperties)
{
    ...
    if (!real_vkEnumerateInstanceExtensionProperties && g_real_vulkan_handle) {
        real_vkEnumerateInstanceExtensionProperties = (PFN_vkEnumerateInstanceExtensionProperties)
            dlsym(g_real_vulkan_handle, "vkEnumerateInstanceExtensionProperties");
    }
    if (!real_vkEnumerateInstanceExtensionProperties) {
        ERROR_LOG("[XCB-Bridge] vkEnumerateInstanceExtensionProperties: no real function!\n");
        return VK_ERROR_EXTENSION_NOT_PRESENT;
    }
    ...
    // Add VK_KHR_surface, VK_EXT_headless_surface, VK_KHR_xcb_surface, VK_KHR_win32_surface
    uint32_t total = real_count + 4;

    if (!pProperties) {
        *pPropertyCount = total;
        fprintf(stderr, "[XCB-Bridge] Extensions: %u (real=%u + surface + headless + xcb + win32)\n", total, real_count);
        return VK_SUCCESS;
    }
    ...
    if (*pPropertyCount > idx) {
        strncpy(pProperties[idx].extensionName, "VK_KHR_win32_surface", VK_MAX_EXTENSION_NAME_SIZE);
        pProperties[idx].specVersion = 6;
        idx++; added++;
    }

    *pPropertyCount = real_count + added;
    fprintf(stderr, "[XCB-Bridge] Added %u extensions (surface + headless + xcb + win32)\n", added);
    return (added == 4) ? VK_SUCCESS : VK_INCOMPLETE;
}
    ...
    VkInstance* pInstance)
{
    ...
    if (!real_vkCreateInstance && g_real_vulkan_handle) {
        real_vkCreateInstance = (PFN_vkCreateInstance)
            dlsym(g_real_vulkan_handle, "vkCreateInstance");
    }
    if (!real_vkCreateInstance) {
        ERROR_LOG("[XCB-Bridge] vkCreateInstance: no real function! (real_handle=%p)\n",
                  g_real_vulkan_handle);
        return VK_ERROR_EXTENSION_NOT_PRESENT;
    }

    ERROR_LOG("[XCB-Bridge] vkCreateInstance called with %u extensions (real_fn=%p)\n",
              pCreateInfo->enabledExtensionCount, (void*)real_vkCreateInstance);

    // Log all requested extensions
    for (uint32_t i = 0; i < pCreateInfo->enabledExtensionCount; i++) {
        ERROR_LOG("[XCB-Bridge]   ext[%u]: %s\n", i, pCreateInfo->ppEnabledExtensionNames[i]);
    }

    // Enumerate extensions the ICD actually supports
    uint32_t icd_ext_count = 0;
    VkExtensionProperties* icd_exts = NULL;
    if (real_vkEnumerateInstanceExtensionProperties ||
        (g_real_vulkan_handle && (real_vkEnumerateInstanceExtensionProperties =
            (PFN_vkEnumerateInstanceExtensionProperties)dlsym(g_real_vulkan_handle,
                "vkEnumerateInstanceExtensionProperties")))) {
        real_vkEnumerateInstanceExtensionProperties(NULL, &icd_ext_count, NULL);
        if (icd_ext_count > 0) {
            icd_exts = malloc(icd_ext_count * sizeof(VkExtensionProperties));
            real_vkEnumerateInstanceExtensionProperties(NULL, &icd_ext_count, icd_exts);
            ERROR_LOG("[XCB-Bridge] ICD supports %u extensions:\n", icd_ext_count);
            for (uint32_t i = 0; i < icd_ext_count; i++) {
                ERROR_LOG("[XCB-Bridge]   icd[%u]: %s\n", i, icd_exts[i].extensionName);
            }
        }
    }

    // Surface extensions we handle ourselves (don't pass to ICD)
    static const char* surface_exts[] = {
        "VK_KHR_surface", "VK_EXT_headless_surface", "VK_KHR_xcb_surface",
        "VK_KHR_xlib_surface", "VK_KHR_wayland_surface", "VK_KHR_win32_surface",
        NULL
    };

    // Filter: keep only extensions the ICD actually supports, strip surface extensions
    ...
    for (uint32_t i = 0; i < pCreateInfo->enabledExtensionCount; i++) {
        ...
        // Skip surface extensions (we handle these)
        int is_surface = 0;
        for (const char** s = surface_exts; *s; s++) {
            if (strcmp(ext, *s) == 0) { is_surface = 1; break; }
        }
        if (is_surface) {
            ERROR_LOG("[XCB-Bridge] Filtering surface ext: %s\n", ext);
            continue;
        }

        // Check if ICD supports this extension
        int icd_supports = 0;
        if (icd_exts) {
            for (uint32_t j = 0; j < icd_ext_count; j++) {
            ...
            icd_supports = 1; // No ICD info, pass through and hope for the best
        }

        if (icd_supports) {
            filtered[filtered_count++] = ext;
        } else {
            ERROR_LOG("[XCB-Bridge] Filtering unsupported ext: %s (Vulkan 1.1+ core promotion?)\n", ext);
        }
    }
    ...
    ERROR_LOG("[XCB-Bridge] >>> Calling real_vkCreateInstance(%u ext, filtered from %u) fn=%p\n",
              filtered_count, pCreateInfo->enabledExtensionCount, (void*)real_vkCreateInstance);
    for (uint32_t i = 0; i < filtered_count; i++) {
        ERROR_LOG("[XCB-Bridge]   pass[%u]: %s\n", i, filtered[i]);
    }

    VkResult result = real_vkCreateInstance(&modified, pAllocator, pInstance);
    free(filtered);

    ERROR_LOG("[XCB-Bridge] <<< real_vkCreateInstance returned %d\n", result);

    if (result == VK_SUCCESS) {
        g_current_instance = *pInstance;
        ERROR_LOG("[XCB-Bridge] Instance created: %p\n", *pInstance);
    } else {
        ERROR_LOG("[XCB-Bridge] Instance creation FAILED: %d\n", result);
    }
    return result;
}
...
PFN_vkVoidFunction vkGetInstanceProcAddr(VkInstance instance, const char* pName)
{
    ...
    if (!real_vkGetInstanceProcAddr && g_real_vulkan_handle) {
    ...
    if (strcmp(pName, "vkCreateWin32SurfaceKHR") == 0)
        return (PFN_vkVoidFunction)my_vkCreateWin32SurfaceKHR;
    if (strcmp(pName, "vkGetPhysicalDeviceWin32PresentationSupportKHR") == 0)
        return (PFN_vkVoidFunction)my_vkGetPhysicalDeviceWin32PresentationSupportKHR;
...
PFN_vkVoidFunction vkGetDeviceProcAddr(VkDevice device, const char* pName)
{
    ...
    if (!real_vkGetDeviceProcAddr && g_real_vulkan_handle) {
        real_vkGetDeviceProcAddr = (PFN_vkGetDeviceProcAddr)
            dlsym(g_real_vulkan_handle, "vkGetDeviceProcAddr");
    }
...
// ============================================================================
// XCB Stubs REMOVED — Wine uses real libxcb via winex11.drv for X11 windowing.
// The Vulkan surface interception above (VK_KHR_xcb_surface → headless) is kept.
// Fake XCB stubs are only needed for standalone vkcube (use without Wine).
// ============================================================================

// ============================================================================
// dlopen interception — Wine loads libvulkan.so.1 via dlopen+dlsym,
// bypassing LD_PRELOAD. We intercept dlopen to return a handle to our own .so
// so that subsequent dlsym calls resolve to our exported wrapper functions.
// ============================================================================

// Real dlopen, resolved once via dlsym(RTLD_NEXT) from the constructor
typedef void* (*PFN_dlopen)(const char*, int);
static PFN_dlopen real_dlopen_fn = NULL;

// Handle to our own .so (set in constructor, used by dlopen override)
static void* g_self_handle = NULL;

// Override dlopen — when Wine opens libvulkan.so.1, return our handle instead
void* dlopen(const char* filename, int flags) {
    if (!real_dlopen_fn) {
        // Should have been set by constructor; fallback just in case
        real_dlopen_fn = (PFN_dlopen)dlsym(RTLD_NEXT, "dlopen");
        if (!real_dlopen_fn) return NULL;
    }

    // Intercept ONLY libvulkan.so.1 / libvulkan.so (the Vulkan loader Wine opens).
    // Do NOT intercept libvulkan_vortek.so, libvulkan_headless.so, etc. —
    // those are ICD libraries that the real loader needs to load normally.
    if (filename && g_self_handle) {
        const char* base = strrchr(filename, '/');
        base = base ? base + 1 : filename;
        if (strcmp(base, "libvulkan.so.1") == 0 || strcmp(base, "libvulkan.so") == 0) {
            // Load the real library so we can resolve real Vulkan functions from it
            void* real_handle = real_dlopen_fn(filename, flags);

            // Save the real handle — we need it to resolve real Vulkan functions,
            // because RTLD_NEXT can't find symbols from dlopen'd libraries.
            if (real_handle && !g_real_vulkan_handle) {
                g_real_vulkan_handle = real_handle;

                // Resolve real Vulkan functions from the actual ICD library
                real_vkGetInstanceProcAddr = (PFN_vkGetInstanceProcAddr)
                    dlsym(real_handle, "vkGetInstanceProcAddr");
                real_vkEnumerateInstanceExtensionProperties = (PFN_vkEnumerateInstanceExtensionProperties)
                    dlsym(real_handle, "vkEnumerateInstanceExtensionProperties");
                real_vkCreateInstance = (PFN_vkCreateInstance)
                    dlsym(real_handle, "vkCreateInstance");
                real_vkGetDeviceProcAddr = (PFN_vkGetDeviceProcAddr)
                    dlsym(real_handle, "vkGetDeviceProcAddr");

                ERROR_LOG("[XCB-Bridge] Resolved real Vulkan functions from %p: "
                          "vkGIPA=%p vkEnum=%p vkCreate=%p vkGDPA=%p\n",
                          real_handle,
                          (void*)real_vkGetInstanceProcAddr,
                          (void*)real_vkEnumerateInstanceExtensionProperties,
                          (void*)real_vkCreateInstance,
                          (void*)real_vkGetDeviceProcAddr);
            }

            // Verify dlsym on our handle finds the key entry point
            void* test = dlsym(g_self_handle, "vkGetInstanceProcAddr");
            ERROR_LOG("[XCB-Bridge] Intercepted dlopen(\"%s\") -> self=%p (real=%p) vkGIPA=%p\n",
                      filename, g_self_handle, real_handle, test);
...
static void init(void) {
    // Resolve real dlopen FIRST (before our override could cause recursion)
    real_dlopen_fn = (PFN_dlopen)dlsym(RTLD_NEXT, "dlopen");

    // Get a handle to our OWN .so by path. dlopen(NULL) returns the global scope
    // which may not resolve our symbols correctly under FEX. Loading by path
    // gives a handle where dlsym finds our exported vkGetInstanceProcAddr etc.
    g_self_handle = real_dlopen_fn("/usr/lib/libvulkan_headless.so", RTLD_NOW | RTLD_NOLOAD);
    if (!g_self_handle) {
        // Fallback: try loading (already loaded via LD_PRELOAD, so just gets a ref)
        g_self_handle = real_dlopen_fn("/usr/lib/libvulkan_headless.so", RTLD_NOW);
    }
    if (!g_self_handle) {
        // Last resort: global scope
        g_self_handle = real_dlopen_fn(NULL, RTLD_NOW);
    }

    ERROR_LOG("[XCB-Bridge] Loaded (pid=%d, self=%p)\n", getpid(), g_self_handle);

    real_vkGetInstanceProcAddr = dlsym(RTLD_NEXT, "vkGetInstanceProcAddr");
    real_vkEnumerateInstanceExtensionProperties = dlsym(RTLD_NEXT, "vkEnumerateInstanceExtensionProperties");
}

#if 0 // XCB stubs disabled — Wine uses real libxcb for X11 windowing
