#!/bin/bash
# Download and install Xvfb + all dependencies into the FEX rootfs on device.
# Run this on your host PC (not on the device).
#
# Usage: ./scripts/install_xvfb.sh

set -e

TMPDIR="/tmp/xvfb_install_$$"
TARBALL="$TMPDIR/xvfb_bundle.tar.gz"
...
echo "=== Step 1: Download Xvfb + all dependencies via Docker ==="
docker run --rm -v "$TMPDIR/debs:/output" ubuntu:22.04 bash -c '
apt-get update -qq 2>/dev/null
apt-get install --download-only -y xvfb xserver-xorg-core 2>/dev/null | tail -3
...
echo "Xvfb binary: $(ls -la /output/usr/bin/Xvfb 2>/dev/null | awk "{print \$5}") bytes"
echo "X modules: $(ls /output/usr/lib/xorg/modules/ 2>/dev/null | wc -l) files"
'
...
adb push "$TARBALL" /data/local/tmp/xvfb_bundle.tar.gz

echo ""
echo "=== Step 5: Extract into rootfs ==="
adb shell "run-as com.mediatek.steamlauncher tar xzf /data/local/tmp/xvfb_bundle.tar.gz -C files/fex-rootfs/Ubuntu_22_04/"
...
echo ""
echo "=== Step 7: Verify Xvfb ==="
adb shell "run-as com.mediatek.steamlauncher ls -la files/fex-rootfs/Ubuntu_22_04/usr/bin/Xvfb"
adb shell "run-as com.mediatek.steamlauncher ls files/fex-rootfs/Ubuntu_22_04/usr/lib/xorg/modules/ 2>/dev/null | head -10"

echo ""
echo "=== Done! ==="
echo "Press 'Start Xvfb' in the app terminal to test."
