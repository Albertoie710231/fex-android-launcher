/**
 * sigill_trap.dll — Vectored Exception Handler for SIGILL diagnosis.
 *
 * Injected into every Wine process via AppInit_DLLs registry key.
 * When EXCEPTION_ILLEGAL_INSTRUCTION fires, dumps:
 ...
 *
 * Compile:
 *   x86_64-w64-mingw32-gcc -shared -o sigill_trap.dll sigill_trap.c -O2 -s
 */
#include <windows.h>
#include <stdio.h>
    ...
    fprintf(stderr, "\nStack (16 QWORDs from RSP):\n");
    {
        ...
        for (j = 0; j < 16; j++) {
            ...
            if (VirtualQuery((LPCVOID)addr_to_check, &mbi, sizeof(mbi)) &&
                ...
                if (sp[j] == 0)
                    fprintf(stderr, "\n");
                /* dump_module_for_addr already adds newline via fprintf */
            } else {
...
BOOL WINAPI DllMain(HINSTANCE hDll, DWORD reason, LPVOID reserved)
{
    if (reason == DLL_PROCESS_ATTACH) {
        ...
        fprintf(stderr, "[sigill_trap] VEH installed in PID %lu (%s)\n",
