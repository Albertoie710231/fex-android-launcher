...
static VkResult trace_CreateImage(void* device, const void* pCreateInfo,
    ...
    /* BC format passthrough — Vortek/Mali handles BCn natively per
     * MEDIATEK-DIRVERS-TEST working state. Previous substitution to RGBA
     * broke Ys IX texture uploads (the game writes BC-sized data but the
     * image was R8G8B8A8, causing data/size mismatches and black textures). */
