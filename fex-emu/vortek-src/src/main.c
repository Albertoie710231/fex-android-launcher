...
static int vortekServerConnect() {
    ...
    // Upstream hardcodes the winlator app dir. Allow override via env so
    // VortekRenderer running inside a different package can be reached.
    const char* env_path = getenv("VORTEK_SERVER_PATH");
    const char* path = (env_path && env_path[0]) ? env_path : VORTEK_SERVER_PATH;
